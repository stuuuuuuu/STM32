...
{
    const portTickType xDelay = pdMS_TO_TICKS(time);
...
bool ESP8266_Net_Mode_Choose(ENUM_Net_ModeTypeDef enumMode)
{
    ...
    case STA_AP:
        return ESP8266_Send_AT_Cmd ( "AT+CWMODE_DEF=3", "OK", "no change", 2500 );
...
bool ESP8266_Link_Server(ENUM_NetPro_TypeDef enumE, char * ip, char * ComNum, ENUM_ID_NO_TypeDef id)
{
    ...
    {
    case enumTCP:
        sprintf ( cStr, "2,\"%s\",\"%s\",%s", "TCP", ip, ComNum );
        break;

    case enumUDP:
        sprintf ( cStr, "2,\"%s\",\"%s\",%s", "UDP", ip, ComNum );
        break;
...
bool ESP8266_SendString(FunctionalState enumEnUnvarnishTx, char * pStr, u32 ulStrLength, ENUM_ID_NO_TypeDef ucId )
{
    ...
    bRet = ESP8266_Send_AT_Cmd ( pStr, "OK", "OK", 1000 );
